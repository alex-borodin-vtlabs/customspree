  ...
  module Api
    class OrdersController < Spree::Api::BaseController
      ...
      def return
        ...
        raise Exception.new(Spree.t(:no_shipments_to_return)) if @order.shipments.blank?
        @order.shipments.each { |shipment| shipment.return! }
        respond_with(@order, :default_template => :show)
      rescue Exception => e
      ...
      def update
        ...
        if @order.contents.update_cart(order_params)
          ...
          if current_api_user.has_spree_role?('admin') && user_id
            ...
            @order.associate_user!(current_api_user)
          end

          begin
            @order.next! if params[:finalize_cart] == "true" && @order.cart?
            respond_with(@order, default_template: :show)
      ...
      def mine
        if current_api_user.persisted?
          if params[:show_incomplete] == "true"
            @orders = current_api_user.orders.reverse_chronological.ransack(params[:q]).result.page(params[:page]).per(params[:per_page])
          else
            @orders = current_api_user.orders.complete.reverse_chronological.ransack(params[:q]).result.page(params[:page]).per(params[:per_page])
          end
        else
