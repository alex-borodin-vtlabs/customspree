  ...
  describe Api::CheckoutsController, type: :controller do
    ...
    context "PUT 'update'" do
      ...
      it "can update shipping method and transition from delivery to payment" do
        ...
        expect(json_response['state']).to eq('confirm') # because of auto-transition
      end
      ...
      it "set the user_id for the order to the user that is authenticated" do
        order.user = nil
        order.save
        api_put :update, id: order.to_param, order_token: order.guest_token,
      ...
      it "can take on the email of the api user (b/c now order is automatically associated to user)" do
        api_put :update, id: order.to_param, order_token: order.guest_token,
          order: { email: "guest@spreecommerce.com" }
        expect(json_response['email']).to eq(current_api_user.email)
        expect(response.status).to eq(200)
      end
    ...
    context "PUT 'next'" do
      ...
      # This spec is no longer valid because we do allow empty emails up until the 'confirm' step
      <<-DOC
