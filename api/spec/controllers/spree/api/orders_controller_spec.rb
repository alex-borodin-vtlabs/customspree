...
module Spree
  describe Api::OrdersController, :type => :controller do
    render_views
    before { Spree::Config[:require_zipcode_validation] = false }
    ...
    context "the current api user is authenticated" do
      ...
      it "can view all of their own orders" do
        api_get :mine, show_incomplete: "true"
      ...
      it "returns only complete orders if 'show_incomplete' param is not passed" do
        order.update_columns completed_at: Time.now

        order2 = Order.create user: order.user, completed_at: Time.now
        order3 = Order.create user: order.user, completed_at: nil
        order4 = Order.create user: order.user, completed_at: nil
    ...
    describe 'return' do
      ...
      it "should return return an error when shipment doesn't exist" do
        order.shipments = []
        api_put :return, :id => order.to_param, :order_token => order.user.authentication_token
        expect(response.status).to eq(422)
        expect(json_response["error"]).to eq(Spree.t(:no_shipments_to_return))
      end
    end
    ...
    context "working with an order" do
      ...
      it "sets the user_id for the order to the user that is authenticated" do
        order.user = nil
        order.save
        user = Spree.user_class.create
        original_id = order.user_id
        api_put :update, :id => order.to_param, :order_token => order.guest_token, order: { email: 'guest@spreecommerce.com' }
        expect(response.status).to eq 200
        expect(json_response["user_id"]).to eq(current_api_user.id)
      end
    ...
    context "as an admin" do
      ...
      context "creation" do
        context "without zipcode validation (i.e. default Spree setup)" do
          it "can create an order without any parameters" do
            expect { api_post :create }.not_to raise_error
            expect(response.status).to eq(201)
            order = Order.last
        ...
        context "with zipcode validation" do
          before { Spree::Config[:require_zipcode_validation] = true }
          let!(:serviceable_zipcode) { create(:serviceable_zipcode, zipcode: '90025') }

          it "cannot create an order without the zipcode" do
            api_post :create
            expect(response.status).to eq(422)
            expect(json_response["error"]).to eq("Validation failed: #{Spree.t(:unserviceable_zipcode)}")
          end

          it "cannot create an order if zipcode is unserviceable" do
            api_post :create, :order => { zipcode: '11111' }
            expect(response.status).to eq(422)
            expect(json_response["error"]).to eq("Validation failed: #{Spree.t(:unserviceable_zipcode)}")
          end

          it "can create an order if zipcode is serviceable" do
            api_post :create, :order => { zipcode: '90025' }
            expect(response.status).to eq(201)
            expect(json_response["state"]).to eq("cart")
          end
        end
      ...
      context "updating" do
        let(:user) { Spree.user_class.create }
        ...
        context "when 'finalize_cart' parameter is passed" do
          it "automatically transitions to the next state" do
            order.state = "cart"
            api_post :update, :id => order.number, finalize_cart: "true", order: { 
              user_id: user.id,
              line_items: { "1": { variant_id: variant.to_param, quantity: 5 } }
            }
