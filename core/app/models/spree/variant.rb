module Spree
  class Variant < Spree::Base
    ...
    delegate_belongs_to :product, :name, :description, :slug, :available_on,
                        ...
                        :shipping_category, :product_properties, :classifications, :taxons
    ...
    has_many :serviceable_zipcodes, through: :stock_locations
    ...
    scope :in_stock, -> { joins(stock_items: :stock_location).where('spree_stock_items.count_on_hand > ? OR track_inventory = ?', 0, false).uniq }
    scope :in_stock_at_location, -> (order_zipcode=nil, scope=nil) do 
      (order_zipcode ? joins(:serviceable_zipcodes).where('spree_serviceable_zipcodes.zipcode = ?', order_zipcode).in_stock : scope || self)
    end

    scope :of_property, -> (property_name, property_value) do 
      with_property(property_name).where("spree_product_properties.value = ?", property_value)
    end

    scope :group_by_property, -> (property_name, order_zipcode=nil) do
      (order_zipcode ? in_stock_at_location(order_zipcode) : self)
        .with_property(property_name)
        .group_by { |variant| variant.property(property_name) }
    end

    scope :with_property, -> (property_name) do 
      joins(product: [:product_properties, :properties]).where("spree_properties.name = ?", property_name).uniq
    end
    ...
    def is_backorderable?(stock_location=nil)
      quantifier(stock_location).backorderable?
    end

    def can_supply?(stock_location=nil, quantity=1)
      quantifier(stock_location).can_supply?(quantity)
    end

    def total_on_hand(stock_location=nil)
      quantifier(stock_location).total_on_hand
    end
    ...
    alias_method :product_type, :property

    private

      def quantifier(stock_location=nil)
        Spree::Stock::Quantifier.new(self, stock_location)
      end
