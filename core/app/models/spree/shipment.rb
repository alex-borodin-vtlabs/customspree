...
module Spree
  class Shipment < Spree::Base
    ...
    state_machine initial: :pending, use_transactions: false do
      ...
      event :return do
        transition to: :return_pending, if: :allow_return?
      end
      after_transition to: :return_pending, do: :after_rental_return_request
      ...
      state :return_in_transit do
        after_save :after_rental_return_in_transit
      end

      state :return_delivered do
        after_save :after_rental_return
      end
    end
    ...
    def after_rental_return
      restock_rental_inventory
      inventory_units.each { |unit| unit.rental_return! if (unit.state != 'rental_returned' && unit.variant.is_rentable?) }
      OrderMailer.final_receipt_email(order.id).deliver_later
    end

    def after_rental_return_request
      order.updater.update_shipment_state
      order.save
    end

    def after_rental_return_in_transit
      OrderMailer.rental_return_confirmation_email(order.id).deliver_later
    end
    ...
    def allow_return?
      ["shipped", "order_delivered"].include? state
    end
    ...
    private
      ...
      def restock_rental_inventory
        manifest.each { |item| manifest_restock(item) if item.variant.is_rentable? }
      end
