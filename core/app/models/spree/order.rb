...
module Spree
  class Order < Spree::Base
    ...
    SHIPMENT_STATES = %w(backorder canceled partial pending ready shipped order_delivered return_pending 
                         return_in_transit return_delivered).freeze
    ...
    self.whitelisted_ransackable_attributes =  %w[completed_at created_at rental_start_date rental_end_date email number state payment_state shipment_state total considered_risky]
    ...
    validate :serviceable_zipcode, on: :update

    delegate :update_totals, :persist_totals, :to => :updater
    delegate :merge!, to: :merger
    delegate :default_credit_card, to: :user
    ...
    def stock_location_id
      Spree::ServiceableZipcode.find_by_zipcode(zipcode).try(:stock_location_id)
    end

    # 8/1/16 to 8/3/16 is considered 3 days not 2
    def rental_days
      if rental_end_date && rental_start_date
        ((rental_end_date - rental_start_date) / 86400).round + 1
      else
        1 # need this just for the sample app b/c items are added before choosing rental start and end dates
      end
    end

    # sum of all rentables - i.e. the "luggages"
    def rental_amount
      rental_days ? line_items.rentable.sum("price * quantity * #{rental_days}") : 0
    end

    # sum of all purchasables - i.e. the "extras"
    def purchase_amount
      line_items.purchasable.sum("price * quantity")
    end
    ...
    def associate_user!(user, override_email = true)
      ...
      changes = slice(:user_id, :email, :created_by_id, :bill_address_id, :ship_address_id, :updated_at)
    ...
    def attempt_auto_transition_to_confirm
      self.next!
    ...
    def ensure_user_has_logged_in
      if user.nil?
        errors.add(:base, Spree.t(:user_needs_to_authenticate))
        false
      else
        true
      end
    ...
    # since there's only one shipping method, Spree will by default use the first and only shipping rate/method available
    def set_default_shipping_method_and_stock_location
      if zipcode
        shipments.update_all(stock_location_id: Spree::ServiceableZipcode.find_by_zipcode(zipcode).try(:stock_location_id))
        self.next
      end
    end
    ...
    def require_email
      true unless new_record? or ['cart', 'address', 'delivery', 'payment'].include?(state)
    end
    ...
    def serviceable_zipcode
      if Spree::Config[:require_zipcode_validation] && !Spree::ServiceableZipcode.find_by_zipcode(zipcode)
        errors.add(:base, Spree.t(:unserviceable_zipcode)) and return false
      end
    end
