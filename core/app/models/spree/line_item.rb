module Spree
  class LineItem < Spree::Base
    ...
    scope :rentable, -> { joins(:variant).where(spree_variants: { is_rentable: true }) }
    scope :purchasable, -> { joins(:variant).where(spree_variants: { is_rentable: false }) }
    ...
    def amount
      (variant && variant.is_rentable?) ? price * quantity * order.rental_days : price * quantity
    end
