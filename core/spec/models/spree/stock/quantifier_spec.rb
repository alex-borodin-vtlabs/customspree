  ...
  module Stock
    describe Quantifier, type: :model do
      ...
      context 'with multiple stock locations/items' do
        let!(:stock_location_2) { create :stock_location, name: 'Los Angeles' }
        ...
        context 'when stock location is not specified' do
          it 'total_on_hand should total all active stock_items' do
            expect(subject.total_on_hand).to eq(15)
          end

          it 'backorderable? should inspect backorders from all active stock_items' do
            expect(subject.backorderable?).to eq(true)
          end
        end

        context 'when stock location is specified' do
          let!(:serviceable_los_angeles_zipcode) { create(:serviceable_zipcode, zipcode: '90025', stock_location: stock_location_2) }
          let!(:serviceable_new_york_zipcode) { create(:serviceable_zipcode, zipcode: '10001', stock_location: stock_location) }
          let(:variant_in_los_angeles) { described_class.new(stock_item.variant, '90025') }
          let(:variant_in_new_york) { described_class.new(stock_item.variant, '10001') }

          it 'total_on_hand should total all active stock_items at given stock location' do
            expect(variant_in_los_angeles.total_on_hand).to eq(5)
          end

          it 'backorderable? should inspect backorders from only the given stock location' do
            expect(variant_in_new_york.backorderable?).to eq(true)
            expect(variant_in_los_angeles.backorderable?).to eq(false)
          end
        end
