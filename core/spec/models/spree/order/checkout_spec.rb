  ...
  context "with default state machine" do
    ...
    context "to delivery" do
      context 'when order has default selected_shipping_rate_id' do
        ...
        let(:shipment) { create(:shipment, order: order, stock_location_id: stock_location.id) }
        ...
        before do
          ...
          order.email = "user@example.com"
          order.bill_address = create(:bill_address)
          order.ship_address = create(:ship_address)
        ...
        context "with zipcode attribute" do
          let(:second_stock_location) { create(:stock_location) }

          before do
            allow(order).to receive_messages :payment_required? => true
            serviceable_zipcode = create(:serviceable_zipcode, zipcode: '90025')
            serviceable_zipcode.stock_location = second_stock_location
            serviceable_zipcode.save!
            order.zipcode = 90025
          end

          it 'should set the stock location to the expected warehouse location' do
            order.next!
            expect(order.shipments.first.stock_location).to eq(second_stock_location)
          end

          it 'should automatically transition to "payment"' do
            order.next!
    ...
    context "from delivery" do
      ...
      context "with payment required" do
        ...
        context "attempt auto transition to confirm" do
          before do
            order.email = 'spree@example.com'
            order.save!
          end

          it "transitions to 'confirm' if payment is already set" do
            order.payments << create(:payment)
            order.next!
            expect(order.state).to eq('confirm')
          end

          it "stays at 'payment' if payment is not set" do
            order.next!
            expect(order.state).to eq('payment')
          end
        end
    ...
    context "to confirm" do
      ...
      context "with unauthenticated user" do
        it "raises a StateMachine::InvalidTransition" do
          ...
          expect(order.errors[:base]).to include(Spree.t(:user_needs_to_authenticate))
        end

        it "remains in 'payment' state" do
          order.next
          expect(order.state).to eq("payment")
        end
      end
    ...
    context "to payment" do
      ...
      it "assigns the user's default credit card" do
        ...
        expect(order.state).to eq 'confirm' # b/c of the auto transition
