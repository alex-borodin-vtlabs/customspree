...
describe Spree::Variant, :type => :model do
  ...
  describe "in_stock_at_location scope" do
    it "returns all in stock variants at particular stock location" do
      los_angeles_stock_location = create(:stock_location, name: 'Los Angeles')
      san_francisco_stock_location = create(:stock_location, name: 'San Francisco')
      los_angeles_serviceable_zipcode = create(:serviceable_zipcode, zipcode: '90025', stock_location: los_angeles_stock_location)
      san_francisco_serviceable_zipcode = create(:serviceable_zipcode, zipcode: '94101', stock_location: san_francisco_stock_location)

      san_francisco_variant = create(:variant)
      los_angeles_variant = create(:variant)
      los_angeles_out_of_stock_variant = create(:variant)

      san_francisco_variant.stock_items.first.update_columns(count_on_hand: 10, stock_location_id: san_francisco_stock_location.id)
      los_angeles_variant.stock_items.first.update_columns(count_on_hand: 10, stock_location_id: los_angeles_stock_location.id)
      los_angeles_out_of_stock_variant.stock_items.first.update_columns(count_on_hand: 0, stock_location_id: los_angeles_stock_location.id)

      expect(Spree::Variant.in_stock_at_location('90025')).to eq [los_angeles_variant]
    end
  end
  ...
  context "property scopes" do
    ...
    let!(:product_property1) { create(:product_property, value: 'luggage', property: property, product: product) }

    describe "of_property scope" do
      it "returns all variants with given property name and value" do
        expect(Spree::Variant.of_property('type', 'luggage')).to eq [product.master]
      end

      it "returns empty array if given property name and value does not match any variants" do
        expect(Spree::Variant.of_property('type', 'foo')).to eq []
        expect(Spree::Variant.of_property('foo', 'luggage')).to eq []
      end
    end

    describe "group_by_property scope" do
      let!(:variant) { create(:base_variant, product: product) }
      let!(:product_property2) { create(:product_property, value: 'electronics', property: property) } # create another product and variant
      let(:los_angeles_stock_location) { create(:stock_location, name: 'Los Angeles') }
      let!(:los_angeles_serviceable_zipcode) { create(:serviceable_zipcode, zipcode: '90025', stock_location: los_angeles_stock_location) }
      let!(:los_angeles_serviceable_zipcode2) { create(:serviceable_zipcode, zipcode: '90001', stock_location: los_angeles_stock_location) }

      context "if stock location is specified" do
        it "returns a hash of variants grouped by property name" do
          variant.stock_items.first.update_columns(count_on_hand: 10, stock_location_id: los_angeles_stock_location.id)
          product_property2.product.master.stock_items.first.update_columns(count_on_hand: 10, stock_location_id: los_angeles_stock_location.id)
          expect(Spree::Variant.group_by_property('type', '90001')).to eq({ "luggage" => [variant], "electronics" => [product_property2.product.master] })
        end
      end

      context "if stock location is not specified" do
        it "returns a hash of variants grouped by property name" do
          expect(Spree::Variant.group_by_property('type')).to eq({ "luggage" => [product.master, variant], "electronics" => [product_property2.product.master] })
        end

        it "returns an empty hash if given property name does not match any variants" do
          expect(Spree::Variant.group_by_property('foo')).to eq({})
        end
      end
    end
  end
