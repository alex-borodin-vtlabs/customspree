...
module Spree
  describe OrderUpdater, type: :model do
    let(:order) { Spree::Order.create(rental_start_date: "2016-3-26", rental_end_date: "2016-3-30") }
    ...
    context "order totals" do
      before do
        ...
        3.times do
          create(:line_item, order: order, price: 8, quantity: 2, variant: create(:rentable_variant))
        end
      end
      ...
      it "update item total" do
        ...
        expect(order.item_total).to eq(212)
      end
      ...
      context 'with order promotion followed by line item addition' do
        ...
        it "updates promotion total" do
          expect(order.promo_total).to eq(-22.2)
        end
      end
