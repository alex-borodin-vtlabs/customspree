...
describe Spree::Shipment, :type => :model do
  ...
  context "#update!" do
    ...
    context "when shipment state changes to shipped" do
      before do
        ...
        allow_any_instance_of(Spree::ShipmentHandler).to receive(:queue_return_reminder_email)
  ...
  context "#return" do
    context "when shipment is ready to be returned" do
      before do
        shipment.state = "order_delivered"
        allow(order).to receive_messages save: true
        allow(order).to receive_message_chain('updater.update_shipment_state' => true)
      end

      it "transitions the state if shipment is ready to be returned" do
        expect(shipment).to receive(:after_rental_return_request)
        shipment.return
        expect(shipment.state).to eq 'return_pending'
      end
    end

    it "doesn't transition the state if shipment is not ready to be returned" do
      shipment.state = "pending"
      shipment.return
    ...
    ['ready', 'canceled'].each do |state|
      context "from #{state}" do
        ...
        it "should send a shipment email" do
          ...
          allow_any_instance_of(Spree::ShipmentHandler).to receive(:update_order_shipment_state)
          allow_any_instance_of(Spree::ShipmentHandler).to receive(:queue_return_reminder_email).and_return(true)

          shipment.ship!
          expect(shipment_id).to eq(shipment.id)
        end

        it "should queue a return reminder email" do
          mail_message = double 'Mail::Message'
          expect(Spree::OrderMailer).to receive(:rental_return_reminder_email) { |*args|
            order_id = order.id
            mail_message
          }
          expect(mail_message).to receive(:deliver_later).with(wait_until: order.rental_end_date - 1.day)
          allow_any_instance_of(Spree::ShipmentHandler).to receive(:update_order_shipment_state)
          allow_any_instance_of(Spree::ShipmentHandler).to receive(:send_shipped_email).and_return(true)
  ...
  context "with return_in_transit state" do
    after do
      shipment.update(state: 'return_in_transit')
    end

    it "sends a return confirmation / in-transit email" do
      mail_message = double 'Mail::Message'
      expect(Spree::OrderMailer).to receive(:rental_return_confirmation_email).with(shipment.order.id).and_return mail_message
      expect(mail_message).to receive :deliver_later
    end
  end

  context "with return_delivered state" do
    after do
      shipment.update(state: 'return_delivered')
    end

    it "restocks the items if inventory units have shipped and are rentals" do
      allow(Spree::OrderMailer).to receive_message_chain('final_receipt_email.deliver_later')
      allow(variant).to receive(:is_rentable?).and_return(true)
      allow(shipment).to receive_message_chain(inventory_units: [mock_model(Spree::InventoryUnit, state: "shipped", line_item: line_item, variant: variant, rental_return!: true)])
      shipment.stock_location = mock_model(Spree::StockLocation)
      expect(shipment.stock_location).to receive(:restock).with(variant, 1, shipment)
      expect(shipment.inventory_units.first).to receive(:rental_return!) 
    end

    it "does not restock the items if inventory units have already been restocked" do
      allow(Spree::OrderMailer).to receive_message_chain('final_receipt_email.deliver_later')
      allow(variant).to receive(:is_rentable?).and_return(true)
      allow(shipment).to receive_message_chain(inventory_units: [mock_model(Spree::InventoryUnit, state: "rental_returned", line_item: line_item, variant: variant, rental_return!: true)])
      shipment.stock_location = mock_model(Spree::StockLocation)
      expect(shipment.stock_location).not_to receive(:restock)
      expect(shipment.inventory_units.first).not_to receive(:rental_return!)
    end

    it "does not restock the items if inventory units are not rentals" do
      allow(Spree::OrderMailer).to receive_message_chain('final_receipt_email.deliver_later')
      allow(variant).to receive(:is_rentable?).and_return(false)
      allow(shipment).to receive_message_chain(inventory_units: [mock_model(Spree::InventoryUnit, state: "shipped", line_item: line_item, variant: variant, rental_return!: true)])
      shipment.stock_location = mock_model(Spree::StockLocation)
      expect(shipment.stock_location).not_to receive(:restock)
      expect(shipment.inventory_units.first).not_to receive(:rental_return!)
    ...
    it "sends a final receipt email" do
      mail_message = double 'Mail::Message'
      expect(Spree::OrderMailer).to receive(:final_receipt_email).with(shipment.order.id).and_return mail_message
      expect(mail_message).to receive :deliver_later
    end
  end
end